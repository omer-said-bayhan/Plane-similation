...
const targetFile = path.join(__dirname, 'game.js');
let code = fs.readFileSync(targetFile, 'utf8');

const newF22 = `function buildF22() {
  ...
  for (let si = -1; si <= 1; si += 2) {
    ...
    // skew the intake face backward slightly
  ...
  // adjust vertices to teardrop shape
...
// also update the renderer to use higher quality tone mapping and shadows
const renderRegex = /const renderer = new THREE\.WebGLRenderer\({[\s\S]*?renderer\.outputColorSpace = THREE\.SRGBColorSpace \|\| "srgb";/;
const renderNew = `const renderer = new THREE.WebGLRenderer({ canvas, antialias:true, powerPreference:"high-performance", logarithmicDepthBuffer: true });
...
let code1 = code.replace(/function buildF22\(\) \{[\s\S]*?return group;\n\}/, newF22);
let code2 = code1.replace(renderRegex, renderNew);

// update lighting
const sunRegex = /const sun = new THREE\.DirectionalLight\(0xfff4d0, 1\.45\);[\s\S]*?scene\.add\(sun\);/;
const sunNew = `const sun = new THREE.DirectionalLight(0xfffaee, 2.0); // Brighter sun
...
let code3 = code2.replace(sunRegex, sunNew);

fs.writeFileSync(targetFile, code3);
console.log("Updated game.js geometry and lighting.");
